...
def generate_comparison_images(
    ...
    post_model, post_ckpt = load_checkpoint(post_ckpt_path, device)
    ...
    for text in texts:
        for sample_idx in range(num_samples_per_text):
            if conditioned:
                pre_deltas = sample_conditioned(pre_model, text, vocab, temperature, device=device)
                post_deltas = sample_conditioned(post_model, text, vocab, temperature, device=device)
            else:
                pre_deltas = sample_unconditional(pre_model, temperature=temperature, device=device)
                post_deltas = sample_unconditional(post_model, temperature=temperature, device=device)
            ...
            for ax, src_fig in zip(axes, [pre_fig, post_fig]):
...
def main() -> None:
    ...
    if demo_ckpt:
        ...
        for text in args.comparison_texts:
            fig = gen.generate_handwriting(text, temperature=args.temperature)
            safe_text = text.replace(" ", "_").replace("/", "_")[:50]
